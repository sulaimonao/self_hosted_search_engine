...
type InstallKind = "chat" | "embedding";
...
const HEALTH_INTERVAL_MS = Number.parseInt(process.env.DESKTOP_PREFLIGHT_HEALTH_INTERVAL ?? "500", 10);
...
async function getCapabilities(): Promise<CapabilitySnapshot> {
  return fetchJson<CapabilitySnapshot>("/api/meta/capabilities", {
    headers: { Accept: "application/json" },
    cache: "no-store",
  });
}

async function installModels(kind: InstallKind, models: string[]): Promise<InstallResponse> {
  const payload: Record<string, string[]> = {};
  payload[kind] = models;
  log(`Requesting installation for ${kind} models: ${models.join(", ")}`);
  return fetchJson<InstallResponse>("/api/admin/install_models", {
    method: "POST",
    headers: {
...
function describeInstallSummary(kind: InstallKind, response: InstallResponse): void {
...
async function waitForCapability(
  label: string,
  predicate: (caps: CapabilitySnapshot) => boolean,
  initialCaps?: CapabilitySnapshot,
...
async function ensureEmbeddingReady(initialCaps: CapabilitySnapshot): Promise<CapabilitySnapshot> {
...
async function ensureChatReady(initialCaps: CapabilitySnapshot): Promise<CapabilitySnapshot> {
