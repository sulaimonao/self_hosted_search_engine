...
class AppConfig:
    ...
    def from_env(cls) -> "AppConfig":
        ...
        normalized_path = _resolve_path(
            os.getenv("NORMALIZED_PATH"), normalized_default
        )
        ...
        ledger_path = _resolve_path(
            os.getenv("INDEX_LEDGER"), data_dir / "index_ledger.json"
        )
        _guard_directory(ledger_path.parent, label="INDEX_LEDGER parent")
        simhash_path = _resolve_path(
            os.getenv("SIMHASH_PATH"), data_dir / "simhash_index.json"
        )
        ...
        _guard_directory(
            last_index_time_path.parent, label="LAST_INDEX_TIME_PATH parent"
        )
        ...
        agent_plans_dir = _guard_directory(
            agent_root / "plans", label="AGENT_PLANS_DIR"
        )
        agent_sessions_dir = _guard_directory(
            agent_root / "sessions", label="AGENT_SESSIONS_DIR"
        )
        ...
        agent_max_fetch_per_turn = max(
            1, int(os.getenv("AGENT_MAX_FETCH_PER_TURN", "3"))
        )
        agent_coverage_threshold = float(os.getenv("AGENT_COVERAGE_THRESHOLD", "0.6"))

        focused_enabled = os.getenv("FOCUSED_CRAWL_ENABLED", "0").lower() in {
            "1",
            "true",
            "yes",
            "on",
        ...
        smart_confidence_threshold = float(
            os.getenv("SMART_CONFIDENCE_THRESHOLD", "0.35")
        )
        smart_seed_min_similarity = float(
            os.getenv("SMART_SEED_MIN_SIMILARITY", "0.35")
        )
        smart_seed_limit = max(0, int(os.getenv("SMART_SEED_LIMIT", "12")))
        search_default_limit = max(1, int(os.getenv("SEARCH_DEFAULT_LIMIT", "20")))
        search_max_limit = max(
            search_default_limit, int(os.getenv("SEARCH_MAX_LIMIT", "50"))
        )
        ...
        use_llm_rerank = os.getenv("USE_LLM_RERANK", "false").lower() in {
            "1",
            "true",
            "yes",
            "on",
    ...
    def log_summary(self) -> None:
        ...
        if not self._ollama_reachable():
            LOGGER.warning(
                "Ollama URL %s is not reachable; LLM enrichment disabled",
                self.ollama_url,
            )
